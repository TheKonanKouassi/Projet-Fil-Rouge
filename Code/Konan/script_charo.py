# ----------------------------------------------------------------------------- #
# DATA CHALLENGE : PREDICTION DE LA CONSOMMATION ELECTRIQUE DE L'ILE D'OUESSANT #
# ----------------------------------------------------------------------------- #
#                            SCRIPT FINAL POUR SOUMISSION                       #
# ----------------------------------------------------------------------------- #
#                                   AUTEURS                                     #
#                            +--------------------+                             #
#                               KASSIR SAROUKOU                                 #
...
class DataFrameSelector(BaseEstimator, TransformerMixin):
	"""
	Cette classe sert de transformateur pour selectionner la liste des variables passée en argument
	"""
...
def series_to_supervised(data, index, n_in = 1, n_out = 1, dropnan = True):
	# Cette fonction permet de faire des version shifté (lag ou delai)
...
bag_reg = BaggingRegressor(ExtraTreesRegressor(), bootstrap = False, n_estimators = 3, random_state = 42)

bag_reg.fit(X_train, y_train)

y_valid_pred = bag_reg.predict(X_valid)
...
plt.ylabel("Réels")
plt.title("Prévisions vs Réels (validation)")
plt.show()

y_test_pred = bag_reg.predict(X_test_)
...
plt.xlabel("Prévisions")
plt.ylabel("Réels")
plt.title("Prévisions vs Réels (test)")
plt.show()
...
# Mettre sous la forme necessaire pour la focntion de lag
...
X_valid = data_total_reframed["2016-09-01":"2016-09-12"].iloc[:, :-1]
y_valid = data_total_reframed["2016-09-01":"2016-09-12"].iloc[:, -1]
...
bag_reg.fit(X_train_prep, y_train)

y_valid_pred = bag_reg.predict(X_valid_prep)
...
print(f"MAE sur test avec reutilisation des prédictions : {mean_absolute_error(y_valid, data_valid_total[:, -1])}")

plt.scatter(data_valid_total[:, -1], y_valid)
plt.plot([0, 1000], [0, 1000], "k--")
plt.xlabel("Prévisions")
plt.ylabel("Réels")
plt.title("Prévisions vs Réels (validation avec reutilisation des prévisions)")
plt.show()

sns.distplot(data_valid_total[:, -1])
plt.xlabel("puissance (Kw)")
plt.ylabel("Effectif")
plt.show()
...
y_train_pred = bag_reg.predict(X_train_prep)

print(f"MAE sur l'ensemble des données :  {mean_absolute_error(y_train_pred, y_train)}")

plt.scatter(y_train_pred, y_train)
plt.xlabel("Prévisions")
plt.ylabel("Réels")
plt.title("Prévisions vs Réels (ensmeble des données)")
plt.show()
...
sns.distplot(data_previ_total[:, -1])
plt.xlabel("puissance (Kw)")
plt.ylabel("Effectif")
plt.show()

y_pred_final = pd.DataFrame(data_previ_total[:, -1], columns = ["puissance"], index = meteo_previ_resampled["2016-09-13":].index)

y_pred_final.plot()
plt.show()

np.savetxt("conso_prev.csv", y_pred_final.values, delimiter = ",")

y_pred_final.to_csv("conso_prev_DataDrame.csv")
